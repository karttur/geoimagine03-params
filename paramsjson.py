...
class RasterPalette():
    ...
    def SetTuplePalette(self, paletteT, paletteName='notest'):
        '''
        '''
        self.paletteT = paletteT
        self.FixGDALPalette()
        ct = ColorTable() 
        if not hasattr(self, 'PcR') or len(paletteT)== 0:
            
            exitstr = "Exiting, empty palette %s" %(paletteName)
...
class Composition:
    ...
    def __init__(self, compD, parameters, system, division, defPath):
        ...
        self._SetExtention(defPath,compD)
    ...
    def _SetVolume(self, defPath):
        ...
        if not path.exists(path.join('/Volumes',self.volume)):
            ...
            exitstr += '\nCheck the volume given in the default parameter file:\n'
            
            exitstr += defaultjsonFPN
            
            exit(exitstr)

    def _SetExtention(self, defPath, compD):
        ''' Set the volume for each composition
        '''
        if 'ext' in compD:
        ...
        for c in copyRec:
            ...
            for item in self.process.dstcopy:
                ...
                for jsonComp in compsL:
                    ...
                    # Set the hdr/ext of the copied composition 
                    srcD['hdr'] = srcD['ext'] = self.process.dstpath.hdr
                    srcD['volume'] = self.process.dstpath.volume
