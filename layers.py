    ...
    def CreateVectorAttributeDef(self,fieldDD): 
        ...
        for key in fieldDD:
            ...
            if 'keyfield' in fieldD:
                ...
                keyfield = False
            if 'source' in fieldD:
                source = fieldD['source']
            else:
                source = None
                
            if 'transfer' in fieldD:
                transfer = fieldD['transfer'].lower()
            else:
                transfer = None
                 
            fieldDefD[key] = {'type':fieldD['type'].lower(), 'width':width, 'precision': precision, 'transfer': transfer, 'source':source, 'keyfield':keyfield}      
        
        for key in fieldDefD:

            self.fieldDefL.append(ktgis.FieldDef(key,fieldDefD[key]))
            
    def _VectorOpenGetFirstLayer(self):
        '''
        '''
        self.DS, self.layer = ktgis.ESRIOpenGetLayer(self.FPN)
        
    def _VectorsCreateDsLayer(self,spatialRef,geomtype,layerid,fieldDefL):
        '''
        '''
        self.DS, self.layer = ktgis.ESRICreateDSLayer(self.FPN,spatialRef,geomtype,layerid,fieldDefL)
         
    def _AddPtDataFromDict(self,xkey,ykey,dictL):
        
        self.layer.AddPtDataFromDict(xkey,ykey,dictL)
